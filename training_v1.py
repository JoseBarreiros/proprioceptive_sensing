...
def intensity_cent(in_order, gray):
	# Intensity at center of circle (numpy uses row by column (so x corresponds
	# to column, y corresponds to row))

	pix_cent = []
	for cent in in_order:
		pix_cent.append(gray[cent[1], cent[0]])

	return pix_cent

def intensity_avg(in_order, test_im, gray):
	# Mean intensity of each circle
	mean_intensity = []
	for cent in in_order:
		circle_img = np.zeros((test_im.shape), np.uint8)
...
def feature_extraction(image, in_order, gray, test_im, size):
	# find intensities
	pix_cent = intensity_cent(in_order, gray)

	

	mean_intensity = intensity_avg(in_order, test_im, gray)

	

	pixels = getPixels(image, size, in_order)

	return (image, pix_cent, mean_intensity, pixels)
