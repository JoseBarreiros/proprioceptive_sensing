...
class Feature(object):
	def __init__(self,frame, gray, pix_cent_gray, pix_cent_bgr, mean_intensity, mean_intensity_bgr, pixels_bgr, pixels_gray,segmentID):
		self.frame=frame
		self.gray=gray
		self.pix_cent_gray=pix_cent_gray
		self.pix_cent_bgr=pix_cent_bgr
		self.mean_intensity=mean_intensity
		self.mean_intensity_bgr=mean_intensity_bgr
...
#		self.coordinate=coordinate			
	...
	def __str__(self):
		...
		rep+= "mean_intensity_bgr:" + str(self.mean_intensity_bgr) + "\n"
...
def getBrightest(norm_image):
	...
	cv2.waitKey(1)	
	#pdb.set_trace()
	coordinates = peak_local_max(im, min_distance=min_dis, threshold_rel=.1)
...
#	pdb.set_trace()
	...
	for coord in in_order:
		...
		cv2.circle(image, coord, circle_radius, (255, 0, 0), 2)
		text = str(z)
		font = cv2.FONT_HERSHEY_SIMPLEX
	...
	for cent in in_order:
		cv2.circle(mask, cent, circle_radius, 255, thickness=-1)
...
def intensity_avg(in_order, gray):
	...
	for cent in in_order:
		...
		circle_radius=30
		cv2.circle(circle_img, cent, circle_radius, (255, 255, 255), thickness=-1)
		#masked_data = cv2.bitwise_and(test_im, test_im, mask=circle_img)

		#cv2.imshow("masked", circle_img)
		#cv2.waitKey(0)

		mean_intensity.append(cv2.mean(gray, mask=circle_img)[0])
		#pdb.set_trace()


	return mean_intensity
def intensity_avg_bgr(in_order, frame,gray):
	# Mean intensity of each circle
	mean_intensity_bgr = []
	#pdb.set_trace()
	for cent in in_order:
		...
		mean_intensity_bgr.append(cv2.mean(frame, mask=circle_img)[:3])

	return mean_intensity_bgr
...
def feature_extraction(frame, com_mask,size):
	...
	#pdb.set_trace()
	mean_intensity_bgr = intensity_avg_bgr(com_mask, frame,gray)

	pixels_bgr = getPixels(frame, size, com_mask)
	pixels_gray = getPixels(gray, size, com_mask)

	return (frame, gray, np.array(pix_cent_gray),np.array(pix_cent_bgr), np.array(mean_intensity),np.array(mean_intensity_bgr), np.array(pixels_bgr),np.array(pixels_gray))


def save_partial(Dtr,i,name_f):


	print("\n\nSaving data for segment: %i" %i)	
	f=open(name_f+"_seg"+str(i)+"_dtr.p","wb")
	pickle.dump(Dtr,f)
	print("Training data for segment %i has been saved sucessfully" %i)
	f.close()
...
def main():
	...
	N = input("How many how many frames for each pocking do you want to acquire? ")
	N=int(N)
	#camera feed
	ret__, frame__ = cap.read()
	cv2.imshow("camera feed",frame__)
	...
	for j in range(0,int(num_trials)):
		...
		while True:
			...
			for kk in range(0,N):
				ret, frame = cap.read()
				frame = cv2.resize(frame, (0,0), fx=0.5, fy=0.5) 
				(frame, gray, pix_cent_gray,pix_cent_bgr, mean_intensity,mean_intensity_bgr, pixels_bgr,pixels_gray)=feature_extraction(frame, com_mask,size)
				temp_feature=Feature(frame, gray, pix_cent_gray,pix_cent_bgr, mean_intensity,mean_intensity_bgr, pixels_bgr,pixels_gray,"SS")
				tempDtr.append(temp_feature)
				time.sleep(0.1)
			...
			if reply==True:		
				...
				#new_feat=Feature(frame_[kk], gray[kk], pix_cent_gray[kk],pix_cent_bgr[kk], mean_intensity[kk],mean_intensity_bgr[kk], pixels_bgr[kk],pixels_gray[kk],"SS")
				Dtr.extend(tempDtr)
				break
	...
	for i in range(0,int(num_seg)):
		...
		for j in range(0,int(num_trials)):
			...
			while True:
				tempDtr=[]
				input("Are you ready to acquire? Press Enter to continue...")
				...
				for kk in range(0,N):
					...
					frame = cv2.resize(frame, (0,0), fx=0.5, fy=0.5) 
					(frame, gray, pix_cent_gray,pix_cent_bgr, mean_intensity,mean_intensity_bgr, pixels_bgr,pixels_gray)=feature_extraction(frame, com_mask,size)
					temp_feature=Feature(frame, gray, pix_cent_gray,pix_cent_bgr, mean_intensity,mean_intensity_bgr, pixels_bgr,pixels_gray,i)
					tempDtr.append(temp_feature)
					time.sleep(0.1)
				...
				if reply==True:		
					...
					#new_feat=Feature(frame_[kk], gray[kk], pix_cent_gray[kk],pix_cent_bgr[kk], mean_intensity[kk], mean_intensity_bgr[kk], pixels_bgr[kk],pixels_gray[kk],i)
					Dtr.extend(tempDtr)
					break
		if i==0:
			name_f = input("\nName your file header: ")

		save_partial(Dtr,i,name_f)
		Dtr=[]
